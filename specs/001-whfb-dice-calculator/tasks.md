...
- [x] T010 [P] Add modifier configurations in `src/entities/dice/model/modifiers.ts` — flat `MODIFIER_CONFIGS: readonly ModifierConfig[]` covering all 11 toHit + 8 toWound + 5 armourSave + 4 wardSave entries from `data-model.md` §2.3 (each card includes one `auto-result` override toggle per FR-T06; To Hit additionally carries two `force-ws` Fear toggles per FR-T07; Strength tiers S4/S5/S6 deliberately omitted from armourSave per FR-018a; Parry Ward lives ONLY on Ward Save card per FR-018 / FR-021), plus an `isStrengthSourceModifier(id: ModifierId): boolean` helper for the cross-card link (depends on T008).
...
- [x] T015 [P] Add unit tests in `src/entities/dice/lib/format.spec.ts` covering: numeric `1+`..`6+` outputs; em-dash + "No save" for AS≥7; em-dash + "No ward" for WSv≥7; em-dash + "Auto-fail" for To Wound ≥ 7 (To Hit clamps to "6+" instead per FR-T05's natural-6 rule); percentage formatting to one decimal place; expected-wounds formatting to three decimal places. Depends on T013.
